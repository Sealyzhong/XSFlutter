...
@interface XSFlutterPlugin : NSObject<FlutterPlugin>

@property(nonatomic,strong) XSFlutterEngine* mxEngine;
...
/// 设置xsflutter jsframework代码的路径
/// [GeneratedPluginRegistrant registerWithRegistry:self]; 运行前设置
+ (void)setJSFramewrokPath:(NSString*)path;


///  jsApp的路径一般直接通过 Flutter XSFlutter.getInstance().runJSApp()直接传入要运行的jsapp路径
