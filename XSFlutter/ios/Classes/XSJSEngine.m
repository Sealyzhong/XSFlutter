...
- (void)setup
{
    self.jsExecutor = [[XSJSExecutor alloc] init];
    
    __weak XSJSEngine *weakSelf = self;
...
- (void)onGCTimer{
    ...
    //TODO: fix bug
    //XSFlutterLog(@"onGCTimer:jsVauleGCManagedMap.count:%lu needDeallocObjMirrorIDArray.count:%lu",
...
- (NSString *)calcRequireJSAbsolutePath:(NSString *)filePath{
    ...
    if ([prefixPath hasPrefix:prefix]) {
        ...
        prefixPath = [NSString stringWithFormat:@"xs_packages/%@",prefixPath];
    ...
    //replace
    NSString *replacePath = [filePath stringByReplacingOccurrencesOfString:@"/packages/" withString:@"/xs_packages/"];
...
- (void)setupBasicJSRuntime:(XSJSEngine* )jsEngine context:(JSContext* )context
{
    __weak XSJSEngine *weakSelf = jsEngine;
    
    context.exceptionHandler = ^(JSContext *con, JSValue *exception) {
        XSFlutterLog(@"js context.exceptionHandler  %@", exception);
    };
    context[@"require"] = ^(NSString *filePath) {
        XSFlutterLog(@"require file:%@",filePath);
        ...
        JSModule *module = nil;
        if (absolutePath.length != 0) {
            //XSFlutterLog(@"require file:%@ found absolutePath=%@",filePath, absolutePath);
    ...
    context[@"XSNativeLog"] = ^(NSString *string) {
        return NSLog(@"%@",string);
    };
    ...
    * @param function 回调
    */
    context[@"xs_jsbridge_MethodChannel_invokeMethod"] = ^(NSString* channelName, NSString* methodName, JSValue* params, JSValue* function) {
...
//    * @param function 回调
//    */
//    context[@"xs_jsbridge_MethodChannel_setMethodCallHandler"] = ^(NSString* channelName, JSValue* function) {
//
//    };
    ...
    */
    context[@"xs_jsbridge_EventChannel_receiveBroadcastStream_listen"] = ^(NSString* channelName,
