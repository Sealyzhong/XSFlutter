...
@implementation XSFlutterEngine
...
- (void)setupChannel
{
    ...
    self.jsFlutterCommonBasicChannel = [FlutterBasicMessageChannel messageChannelWithName:@"xsflutter_common_basic_channel"
                                                                          binaryMessenger:self.binaryMessenger
...
- (void)callNativeRunJSApp:(id)arguments
{
    ...
    if (self.currentJSAppPath.length > 0) {
        XSFlutterLog(@"XSFlutterEngine Native工程，设置了currentJSAppPath，RunJSApp使用Native设置的路径 appPath %@",self.currentJSAppPath);
    ...
    else
    {
        ...
        if (jsAppPath.length  == 0 &&  jsAppAssetsKey.length > 0) {
            NSString *key =  [[XSFlutterPlugin shareInstance].flutterRegistrar lookupKeyForAsset:jsAppAssetsKey];
            jsAppPath = [[[NSBundle mainBundle] bundlePath] stringByAppendingPathComponent:key];
        }
        
        if(jsAppPath.length == 0){
            
            XSFlutterLog(@"%@",@"jsAppPath.length == 0");
            return;
        }
        ...
        if (jsAppSearchPathWithAssetsKeyList.count > 0) {
            ...
            for (NSString *jsAppPathAssetsKey in jsAppSearchPathWithAssetsKeyList) {
                NSString *key =  [[XSFlutterPlugin shareInstance].flutterRegistrar lookupKeyForAsset:jsAppPathAssetsKey];
...
{
    [self callFlutterReloadAppWithRouteName:@"XSJSWidget" widgetData:widgetData];
}
...
- (void)invokeFlutterRemoveMirrorObjsRef:(NSArray*)mirrorIDArray{
    ...
    [self.engineMethodChannel invokeMethod:@"xsfJSBridgeRemoveMirrorObjsRef" arguments:mirrorIDArray];
}
...
- (void)callFlutterMethodChannelInvoke:(NSString*)channelName methodName:(NSString*)methodName params:(NSDictionary *)params callback:(void(^)(id _Nullable result))callback
{
    ...
    FlutterMethodCall* call = [FlutterMethodCall methodCallWithMethodName:@"xsflutterBridgeMethodChannelInvoke" arguments:arguments];
...

{
    ...
    FlutterMethodCall* call = [FlutterMethodCall methodCallWithMethodName:@"xsflutterBridgeEventChannelReceiveBroadcastStreamListenInvoke"
                                                                arguments:arguments];
...
- (void)runAppWithPath:(NSString *)jsAppPath jsAppSearchPathList:(NSArray*)pathArray
{
    ...
    self.currentApp  = [[XSFlutterApp alloc] initWithAppPath:appRootPath jsAppSearchPathList:pathArray engine:self];
